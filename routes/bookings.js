...
bookingRouter.post(
  "/cancel-booking-precheck",
  BookingController.cancelBookingPrecheck
);
...
bookingRouter.put("/update-status/:id", async (req, res, next) => {
  try {
    ...
    if (status === "completed") {
      await transactionDao.transferMoneyBooking(
        updatedBooking.userId._id.toString(),
        "Hoàn tiền",
        "Thành công",
        updatedBooking.totalAmount,
        `Hoàn tiền ${updatedBooking.spaceId.name}`
      );
...
bookingRouter.get("/near-spaces", async (req, res) => {
  try {
    ...
    if (lat && lng) {
      ...
      result = await Spaces.find().sort({ updatedAt: -1 }).limit(3);
    }
    ...
    for (let i = 0; i < result.length; i++) {
      resData.push({
        ...
        pricePerHour: result[i].pricePerHour,
      });
      try {
        const res = await axios.get(
          `https://api.mapbox.com/directions/v5/mapbox/driving/${lng},${lat};${result[i].locationPoint.coordinates[0]},${result[i].locationPoint.coordinates[1]}`,
          {
            params: {
        ...
        if (
          ...
          resData[i].distance =
            res.data.routes[0].distance >= 1000
              ? `${Math.floor(res.data.routes[0].distance / 1000)}km`
              : `${res.data.routes[0].distance}m`;
        }
      } catch (error) {
        console.log(error.message);
      }
